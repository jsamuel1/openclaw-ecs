...
export class ClawdbotEcsStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: ClawdbotEcsStackProps) {
    ...
    const container = taskDefinition.addContainer('clawdbot', {
      ...
      command: [
        'sh', '-c',
        'apt-get update && apt-get install -y git && git clone https://github.com/jsamuel1/clawdbot.git /tmp/clawdbot && cd /tmp/clawdbot && npm install -g . && clawdbot gateway run --bind 0.0.0.0 --port 18789',
      ],
      environment: {
        HOME: '/data',
        NODE_ENV: 'production',
